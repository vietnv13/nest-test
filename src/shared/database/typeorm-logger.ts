...
import { Logger as TypeORMLoggerInterface, LoggerOptions, QueryRunner } from 'typeorm';
...
export class TypeORMLogger implements TypeORMLoggerInterface {
  ...
  private isEnable(level: 'query' | 'schema' | 'error' | 'warn' | 'info' | 'log' | 'migration'): boolean {
    return (
      this.options === 'all' || this.options === true || (Array.isArray(this.options) && this.options.includes(level))
    );
  }
}
